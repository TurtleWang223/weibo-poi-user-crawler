...
def get_poi_res(pageinfo_url):
    ...
    try:
        ...
        print(f'因为{ne}报错的pageinfo_url为{pageinfo_url}')
    except IndexError as ne2:
        print(f'因为{ne2}报错的pageinfo_url为{pageinfo_url}')

    poi_res_2 = requests.get(url=f'https://place.weibo.com/wandermap/pois?&poiid={poi_id}', headers=headers,
                             timeout=5).json()

    return poi_res_1, poi_res_2, poi_id
...
def get_information(blog_res, poi_res_1, poi_res_2, user_res_1, user_res_2, city):
    ...
    try:
        ...
        poi_res_1['data']['cards'][0]['card_group'][0]['group'][0]['item_desc'].split(' ')[0]
    except:
        poi_user_dict['poi_category'] = ''
    try:
...
def connect_mysql(poi_user_dict, city):
    ...
    try:
        ...
        with pymysql.connect(**config) as connection:
            ...
            connection.commit()
            print(
                f"成功插入poi_id为{poi_user_dict['poi_id']}，用户名为：{poi_user_dict['user_name']}，时间为{datetime.datetime.strptime(poi_user_dict['blog_time_stamp'], '%a %b %d %H:%M:%S %z %Y')}的记录")
...
if __name__ == '__main__':
    ...
    while flag:
        ...
        for blog_res in (
        city_res['data']['cards'][1]['card_group'] if since_id == 1 else city_res['data']['cards'][0]['card_group']):
            ...
            try:
                ...
                continue
            # 获取poi信息和user信息
            poi_res_1, poi_res_2, poi_id = get_poi_res(pageinfo_url)
            if poi_id == '':
                print("poi_id为空的数据属于无效数据，跳过")
                continue
